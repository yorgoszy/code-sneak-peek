...
import { Target, Shield, Clock, TrendingUp, Users, Swords, Activity, Film, Calendar, MapPin, User, Eye, Edit, Trash2, Loader2, Trophy, CheckCircle, CirclePlus } from 'lucide-react';
        ...
        <div className="flex items-center gap-2 flex-1">
          ...
          >
            <div className="relative">
              ...
              <CirclePlus className="w-2.5 h-2.5 absolute -bottom-0.5 -right-0.5 text-[#00ffba]" />
            </div>
          </Button>
      ...
      {/* Stats Cards - All rows with minimal gap */}
          <div className="space-y-0.5 md:space-y-2">
            ...
            <div className="grid grid-cols-4 gap-0.5 md:gap-2">
              {statCards2.slice(0, 4).map((card, index) => (
            ...
            <div className="grid grid-cols-2 gap-0.5 md:gap-2">
              {statCards2.slice(4).map((card, index) => (
                <Card key={index} className={`rounded-none transition-all ${selectedFightId ? 'ring-1 ring-[#00ffba]/10' : 'opacity-50'}`}>
                  <CardContent className="p-1 md:p-2">
                    <div className="flex items-center gap-1 md:gap-2">
                      {(card as any).imageIcon ? (
                  ...
                  {(() => {
                    ...
                    return (
                      <span className="font-medium">
                        <span className={`${isBlue ? 'text-red-500 text-xs' : 'text-red-500 text-base font-bold border border-current px-1.5 py-0.5'}`}>{isBlue ? oppName : userName}</span>
                        <span className="text-gray-400 mx-1 text-sm">vs</span>
                        <span className={`${isBlue ? 'text-blue-500 text-base font-bold border border-current px-1.5 py-0.5' : 'text-blue-500 text-xs'}`}>{isBlue ? userName : oppName}</span>
                      </span>
                    );
                  })()}
                                ...
                                return (
                                  <>
                                    ...
                                    <div className="flex items-center gap-2 flex-wrap">
                                      <span className={`truncate text-red-500 ${topIsOurs ? 'text-base font-bold border border-current px-1.5 py-0.5' : 'text-xs font-medium'}`}>{topName}</span>
                                      <span className="text-xs text-gray-400">vs</span>
                                      <span className={`truncate text-blue-500 ${bottomIsOurs ? 'text-base font-bold border border-current px-1.5 py-0.5' : 'text-xs font-medium'}`}>{bottomName}</span>
                                    </div>
                                    {/* Line 2: Result + meta */}
