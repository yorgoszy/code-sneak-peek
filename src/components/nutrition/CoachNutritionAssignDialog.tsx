...
interface CoachNutritionAssignDialogProps {
  isOpen: boolean;
  onClose: () => void;
  plan: {
    id: string;
...
export const CoachNutritionAssignDialog: React.FC<CoachNutritionAssignDialogProps> = ({
  isOpen,
  onClose,
  plan,
  onSuccess
  ...
  const fetchCoachAthletes = async () => {
    ...
    try {
      // Fetch only athletes assigned to this coach
      ...
      <DialogContent className="max-w-md rounded-none">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label>Επιλέξτε Αθλητή</Label>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
              <Input
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                placeholder="Αναζήτηση αθλητή με όνομα ή email..."
            ...
            {users.length === 0 ? (
              <p className="text-sm text-gray-500 text-center py-4">
                Δεν έχετε αθλητές ανατεθειμένους σε εσάς
              </p>
            ) : (
          ...
          {selectedUser && (
            <Card className="rounded-none bg-[#00ffba]/5 border-[#00ffba]">
              <CardContent className="p-3 text-sm">
                <p className="font-medium">Επιλεγμένος αθλητής: {selectedUser.name}</p>
