  ...
  return (
    <div className="space-y-2 sm:space-y-0 px-2 sm:px-0">
      {historicalData.length > 0 ? (
                      ...
                      >
                        ...
                        {oneRM.history && oneRM.history.length > 0 && (
                          <div className="space-y-0.5 pt-1.5 sm:pt-1 border-t border-gray-200 mt-1.5 sm:mt-1">
                            ...
                            {oneRM.history.map((hist, idx) => (
                              <div key={idx} className="flex items-center justify-between text-xs sm:text-[9px] text-gray-400">
                                <span>{new Date(hist.date).toLocaleDateString('el-GR', { day: '2-digit', month: '2-digit', year: '2-digit' })}</span>
                                <span>{hist.weight}kg @ {hist.velocity.toFixed(2)}m/s</span>
                              </div>
                            ))}
                          </div>
                        )}
        ...
        <div className="text-center py-8 text-sm sm:text-base text-gray-500">
          Δεν υπάρχουν δεδομένα Force/Velocity
        </div>
      )}
    </div>
  );
