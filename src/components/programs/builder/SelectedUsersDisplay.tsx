
import React from 'react';
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Avatar, AvatarImage, AvatarFallback } from "@/components/ui/avatar";
import { Users, X } from "lucide-react";
import type { User as UserType } from '../types';

interface SelectedUsersDisplayProps {
  selectedUsers: UserType[];
  onClearAll: () => void;
  onRemoveUser: (userId: string, event: React.MouseEvent) => void;
}

export const SelectedUsersDisplay: React.FC<SelectedUsersDisplayProps> = ({
  selectedUsers,
  onClearAll,
  onRemoveUser
}) => {
  const getUserInitials = (name: string) => {
    return name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2);
  };

  return (
    <Card className="rounded-none">
      <CardHeader className="py-2 px-3">
        <CardTitle className="flex items-center justify-between text-xs">
          <span className="flex items-center gap-1.5">
            <Users className="w-3.5 h-3.5" />
            Επιλεγμένοι ({selectedUsers.length})
          </span>
          {selectedUsers.length > 0 && (
            <Button
              variant="outline"
              size="sm"
              onClick={onClearAll}
              className="rounded-none text-xs h-5 px-1.5"
            >
              Καθαρισμός
            </Button>
          )}
        </CardTitle>
      </CardHeader>
      <CardContent className="py-2">
        {selectedUsers.length === 0 ? (
          <div className="text-center text-sm text-gray-500 py-2">
            Δεν έχουν επιλεγεί χρήστες
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-1.5">
            {selectedUsers.map(user => (
              <div
                key={user.id}
                className="flex items-center justify-between bg-[#00ffba]/10 border border-[#00ffba]/20 p-1.5 rounded hover:bg-[#00ffba]/20 transition-colors"
              >
                <div className="flex items-center gap-1.5 min-w-0 flex-1">
                  <Avatar className="w-5 h-5 flex-shrink-0">
                    <AvatarImage src={user.photo_url || user.avatar_url || ""} alt={user.name} />
                    <AvatarFallback className="text-[10px]">
                      {getUserInitials(user.name)}
                    </AvatarFallback>
                  </Avatar>
                  <p className="font-medium text-xs truncate">{user.name}</p>
                </div>
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={(e) => onRemoveUser(user.id, e)}
                  className="rounded-none p-0.5 h-auto text-gray-500 hover:text-red-600 hover:bg-red-50 flex-shrink-0"
                  title="Αφαίρεση"
                >
                  <X className="w-3 h-3" />
                </Button>
              </div>
            ))}
          </div>
        )}
      </CardContent>
    </Card>
  );
};
