...
export const OfferEditDialog: React.FC<OfferEditDialogProps> = ({
  isOpen,
  onClose,
  onSuccess,
  offer
  ...
  const populateForm = () => {
    if (offer) {
      ...
      setSubscriptionTypeId(offer.subscription_type_id || '');
      setDiscountedPrice(offer.discounted_price?.toString() || '');
      setStartDate(offer.start_date ? new Date(offer.start_date) : undefined);
      setEndDate(offer.end_date ? new Date(offer.end_date) : undefined);
      setVisibility(offer.visibility || 'all');
      setSelectedUsers(offer.target_users || []);
      setSelectedGroups(offer.target_groups || []);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error('Σφάλμα κατά την ενημέρωση της προσφοράς');
    } finally {
      setSaving(false);
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto rounded-none">
        ...
        <div className="flex justify-end space-x-2 pt-4">
          ...
          >
            {saving ? 'Ενημέρωση...' : 'Ενημέρωση Προσφοράς'}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
