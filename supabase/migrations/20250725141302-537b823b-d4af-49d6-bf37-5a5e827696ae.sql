-- Update get_user_available_bookings to include allowed sections from subscriptions
...
BEGIN
  ...
  SELECT us.*, st.name, st.visit_count, st.allowed_sections
  ...
  SELECT us.*, st.name, st.allowed_sections
  ...
  ELSIF visit_packages_total > 0 THEN
    -- For visit packages, get allowed sections from the most recent active subscription that created packages
    SELECT st.allowed_sections INTO user_allowed_sections
    FROM public.user_subscriptions us
    JOIN public.subscription_types st ON us.subscription_type_id = st.id
    WHERE us.user_id = user_uuid
      AND st.subscription_mode = 'visit_based'
      AND us.status = 'active'
