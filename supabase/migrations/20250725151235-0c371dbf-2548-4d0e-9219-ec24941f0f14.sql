-- Update get_user_available_bookings function to properly handle visit packages without expiry date
...
BEGIN
  ...
  SELECT 
    ...
    MAX(expiry_date)
  INTO visit_packages_total, visit_packages_used, visit_package_expiry
  ...
  -- Determine allowed sections and subscription end date based on subscription type
  ...
  ELSIF visit_packages_total > 0 THEN
    ...
    -- Only set subscription_end_date if visit packages have expiry dates
    -- If packages have no expiry (NULL), don't restrict booking dates
    subscription_end_date := visit_package_expiry;
